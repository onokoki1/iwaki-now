const fallbackNews = [
  ...
  {id:"seed-7",title:"勿来・國玉神社の「風鈴参道」　8月16日まで",summary:"勿来町窪田の國玉神社では、風鈴とこけ玉で彩る「風鈴参道」を開催中です。",category:"イベント",area:"勿来",publishedAt:"2026-07-28T11:00:00+09:00",source:"いわき民報",sourceUrl:"https://iwaki-minpo.co.jp/",priorityScore:16,coverageCount:1},
...
let news=fallbackNews,generatedAt=null,sourceCount=null;
const state={category:"すべて",area:"すべて",query:"",sort:"new"};
...
function filteredNews(){return sortedNews().filter(n=>(state.category==='すべて'||n.category===state.category)&&(state.area==='すべて'||n.area===state.area||n.area==='全市')&&(!state.query||`${n.title} ${n.summary||''} ${n.category} ${n.area} ${n.source}`.toLowerCase().includes(state.query.toLowerCase())))}
...
function cardMarkup(n){
  ...
  return `<article class="news-card"><div class="news-thumb" data-category="${esc(n.category)}"><span class="thumb-badge">${esc(n.category)}</span>${breaking}</div><div class="news-card-body"><div class="card-badges">${cov}</div><h3><a href="${detailUrl(n)}">${esc(n.title)}</a></h3><p>${esc(n.summary||'')}</p><div class="news-card-footer"><div class="source"><b>${esc(n.area)}</b>${esc(n.source)}　${timeAgo(n.publishedAt)}</div><a class="read-button" href="${detailUrl(n)}" aria-label="記事詳細を開く">→</a></div></div></article>`
}
function renderNews(){const items=filteredNews();$('#newsGrid').innerHTML=items.map(cardMarkup).join('');$('#resultCount').textContent=`${items.length}件`;$('#emptyState').hidden=items.length!==0;updateRegionStates()}
...
function syncFilters(){if($('#categoryFilter'))$('#categoryFilter').value=state.category;if($('#areaFilter'))$('#areaFilter').value=state.area;if($('#searchInput'))$('#searchInput').value=state.query;if($('#headerSearchInput'))$('#headerSearchInput').value=state.query;$$('.nav-chip').forEach(b=>b.classList.toggle('active',b.dataset.category===state.category));$('#allFilter')?.classList.toggle('active',state.category==='すべて'&&state.area==='すべて'&&!state.query);renderNews()}
function selectArea(area){state.area=area;syncFilters();$('#latestTitle')?.scrollIntoView({behavior:'smooth',block:'start'})}
function selectCategory(cat){state.category=cat;syncFilters();$('#latestTitle')?.scrollIntoView({behavior:'smooth',block:'start'})}
...
async function loadNews(){
  try{
    ...
    if(Array.isArray(p.news)&&p.news.length){news=p.news;generatedAt=p.generatedAt||null;sourceCount=p.sourceCount||(Array.isArray(p.sources)?p.sources.length:null)}
  }catch(e){console.info('内蔵デモデータを表示します',e)}
  renderBreaking();renderTopFive();renderNews();renderMeta();refreshRegionButtons();
}
...
function bindEvents(){
  ...
  $('#categoryFilter').addEventListener('change',e=>{state.category=e.target.value;syncFilters()});
  $('#areaFilter').addEventListener('change',e=>{state.area=e.target.value;syncFilters()});
  $('#sortFilter').addEventListener('change',e=>{state.sort=e.target.value;renderNews()});
  for(const id of ['searchInput','headerSearchInput'])$("#"+id).addEventListener('input',e=>{state.query=e.target.value;syncFilters()});
  $('#clearFilters').addEventListener('click',()=>{Object.assign(state,{category:'すべて',area:'すべて',query:'',sort:'new'});$('#sortFilter').value='new';syncFilters()});
  $('#allFilter').addEventListener('click',()=>{Object.assign(state,{category:'すべて',area:'すべて',query:''});syncFilters()});
  $('#resetRegion').addEventListener('click',()=>{state.area='すべて';syncFilters()});
